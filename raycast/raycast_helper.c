...
void adjust_hit_tile(t_var *data, t_ray *ray)
{
    ...
    if (cur == DOORH_CLOSE)
    {
        if ((data->player.py - ((int)data->player.py / TILE_SIZE * TILE_SIZE) > (TILE_SIZE - DOORWIDTH) / 2 && ray->ra >= PI) ||
            (data->player.py - ((int)data->player.py / TILE_SIZE * TILE_SIZE) < (TILE_SIZE - DOORWIDTH) / 2 && ray->ra <= PI))
        {
            ray->rx -= ray->xo;
            ray->ry -= ray->yo;
            ray->dof -= 1;
    ...
    {
        if ((data->player.px - ((int)data->player.px / TILE_SIZE * TILE_SIZE) > (TILE_SIZE - DOORWIDTH) / 2 && (ray->ra > PI/2 && ray->ra < 1.5 * PI)) ||
            (data->player.px - ((int)data->player.px / TILE_SIZE * TILE_SIZE) < (TILE_SIZE - DOORWIDTH) / 2 && !(ray->ra > PI/2 && ray->ra < 1.5 * PI)))
        {
            ray->rx -= ray->xo;
            ray->ry -= ray->yo;
            ray->dof -= 1;
